 ...
 * bounceBar pointing forward horizontal is 0
 * bounceBar points toward the back of the robot so endstops are about 100-170 degrees
 * 
 */
public class BounceBarS extends SubsystemBase implements Logged {
   ...
   * For visualization.
   */
  public final MechanismLigament2d BOUNCE_BAR = new MechanismLigament2d(
    "bouncebar", Units.inchesToMeters(6), 0, 4, new Color8Bit(235, 137, 52));

  /** Creates a new bounceBarS. */
  public BounceBarS() {
    // Create the IO class.
    if (Robot.isSimulation()) {
      m_io = new SimBounceBarIO();
      isHomed = new Trigger(()-> true );
    }
    else {
      m_io = new RealBounceBarIO();
        isHomed = new Trigger(()-> getCurrent()>10).debounce(0.6995);
    }
    m_profile = new ExponentialProfile(Constants.CONSTRAINTS);
    BOUNCE_BAR.append(BOUNCE_BAR);
  ...
  public void periodic() {
    ...
    BOUNCE_BAR.setAngle(Units.radiansToDegrees(m_io.getAngle() + Units.degreesToRadians(55-14) ));
  ...
  public SysIdRoutine m_idRoutine = new SysIdRoutine(
    ...
    new Mechanism(
      ...
      (log)->{
        log.motor("bounceBar").angularPosition(
