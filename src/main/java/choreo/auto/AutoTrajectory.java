...
import choreo.auto.AutoFactory.TriConsumer;
...
public class AutoTrajectory {
  ...
  private final TriConsumer<Boolean, Pose2d, ? extends TrajectorySample<?>> end;
  ...
  <SampleType extends TrajectorySample<SampleType>> AutoTrajectory(
      ...
      TriConsumer<Boolean, Pose2d, SampleType> end,
  ...
  private void cmdEnd(boolean interrupted) {
    timer.stop();
      if (currentSample instanceof SwerveSample) {
        TriConsumer<Boolean, Pose2d, SwerveSample> swerveController =
            (TriConsumer<Boolean, Pose2d, SwerveSample>) this.end;
        SwerveSample swerveSample = (SwerveSample) currentSample;
        swerveController.accept(interrupted,currentSample.getPose(), swerveSample);
      } else if (currentSample instanceof DifferentialSample) {
        TriConsumer<Boolean, Pose2d, DifferentialSample> differentialController =
            (TriConsumer<Boolean, Pose2d, DifferentialSample>) this.end;
        DifferentialSample differentialSample = (DifferentialSample) currentSample;
        differentialController.accept(interrupted, currentSample.getPose(), differentialSample);
      }
    isActive = false;
    logTrajectory(false);
  }
